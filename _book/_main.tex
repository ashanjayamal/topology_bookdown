...
\begin{exercise}
\protect\hypertarget{exr:unnamed-chunk-98}{}\label{exr:unnamed-chunk-98}A map \(f : X \to Y\). We say that \(f\) is an \textbf{open map} if, for every open set \(U\) in \(X\), the set \(f(U)\) is open in \(Y\). Show that the projections \(\pi_1 : X \times Y \to X\) and \(\pi_2 : X \times Y \to Y\) are open maps.
``
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-100}{}\label{exr:unnamed-chunk-100}Show that the countable collection
\[\{(a, b) \times (c, d) | a < b \text{ and } c < d, \text{ and } a, b, c, d \text{ are rational}\}\]
is a basis for \(\mathbb{R}^2\).''
\end{exercise}

\begin{exercise}
\protect\hypertarget{exr:unnamed-chunk-101}{}\label{exr:unnamed-chunk-101}Let \(X\) be an ordered set. If \(Y\) is a proper subset of \(X\) that is convex in \(X\), does it follow that \(Y\) is an interval or a ray in \(X\)?
\end{exercise}

\begin{exercise}
\protect\hypertarget{exr:unnamed-chunk-102}{}\label{exr:unnamed-chunk-102}If \(L\) is a straight line in the plane, describe the topology \(L\) inherits as a subspace of \(\mathbb{R}_l \times \mathbb{R}\) and as a subspace of \(\mathbb{R}_l \times \mathbb{R}_l\). In each case it is a familiar topology.
\end{exercise}

\begin{exercise}
\protect\hypertarget{exr:unnamed-chunk-103}{}\label{exr:unnamed-chunk-103}Show that the dictionary order topology on the set \(\mathbb{R} \times \mathbb{R}\) is the same as the product topology \(\mathbb{R}_d \times \mathbb{R}\), where \(\mathbb{R}_d\) denotes \(\mathbb{R}\) in the discrete topology. Compare this topology with the standard topology on \(\mathbb{R}^2\).
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-105}{}\label{exr:unnamed-chunk-105}Let \(\mathcal{C}\) be a collection of subsets of the set \(X\). Suppose that \$\$\emptyset and \(X\) are in \(\mathcal{C}\), and that finite unions and arbitrary intersections of elements of \(\mathcal{C}\) are in \(\mathcal{C}\). Show that the collection
...
\protect\hypertarget{exr:unnamed-chunk-107}{}\label{exr:unnamed-chunk-107}Show that if \(A\) is closed in \(Y\) and \(Y\) is closed in \(X\), then \(A\) is closed in \(X\).
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-109}{}\label{exr:unnamed-chunk-109}Show that if \(A\) is closed in \(X\) and \(B\) is closed in \(Y\) , then \(A\times B\) is closed in \(X\times Y\)
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-111}{}\label{exr:unnamed-chunk-111}Show that if \(U\) is open in \(X\) and \(A\) is closed in \(X\), then \(U \setminus A\) is open in \(X\), and \(A \setminus U\) is closed in \(X\).
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-113}{}\label{exr:unnamed-chunk-113}Let \(X\) be an ordered set in the order topology. Show that \(\overline{(a, b)} \subseteq [a, b]\). Under what conditions does equality hold?
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-117}{}\label{exr:unnamed-chunk-117}Criticize the following ``proof'' that \(\overline{\cup A_\alpha} \subset \cup \overline{A_\alpha}\):\\
...
\protect\hypertarget{exr:unnamed-chunk-118}{}\label{exr:unnamed-chunk-118}Let \(A\), \(B\), and \(A_{\alpha}\) denote subsets of a space \(X\). Determine whether the following equations hold; if an equality fails, determine whether one of the inclusions \(\supset\) or \(\subset\) holds.
...
\protect\hypertarget{exr:unnamed-chunk-123}{}\label{exr:unnamed-chunk-123}Let \(A \subset X\) and \(B \subset Y\). Show that in the space \(X \times Y\),
...
\protect\hypertarget{exr:unnamed-chunk-125}{}\label{exr:unnamed-chunk-125}Show that every order topology is Hausdorff.
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-10.png}

\begin{exercise}[Mun 2.17.11]
\protect\hypertarget{exr:unnamed-chunk-127}{}\label{exr:unnamed-chunk-127}Show that the product of two Hausdorff spaces is Hausdorff.
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-11.png}

\begin{exercise}[Mun 2.17.12]
\protect\hypertarget{exr:unnamed-chunk-129}{}\label{exr:unnamed-chunk-129}Show that a subspace of a Hausdorff space is Hausdorff.
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-12.png}

\begin{exercise}[Mun 2.17.13]
\protect\hypertarget{exr:unnamed-chunk-131}{}\label{exr:unnamed-chunk-131}Show that \(X\) is Hausdorff if and only if the diagonal \(\Delta = \{x\times x | x \in X\}\) is closed in \(X \times X\).
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-13.png}

\begin{exercise}[Mun 2.17.14]
\protect\hypertarget{exr:unnamed-chunk-133}{}\label{exr:unnamed-chunk-133}In the finite complement topology on \(\mathbb{R}\), to what point or points does the sequence \(x_n = \frac{1}{n}\) converge?
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-14.png}

\begin{exercise}[Mun 2.17.15]
\protect\hypertarget{exr:unnamed-chunk-135}{}\label{exr:unnamed-chunk-135}Show the \(T_1\) axiom is equivalent to the condition that for each pair of points of \(X\), each has a neighborhood not containing the other.
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-140}{}\label{exr:unnamed-chunk-140}Consider the lower limit topology on \(\mathbb{R}\) and the topology given by the basis \(C\) of Exercise 8 of §13. Determine the closures of the intervals \(A = (0, \sqrt{2})\) and \(B = (\sqrt{2}, 3)\) in these two topologies.
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/Notsure.png}

\begin{exercise}[Mun 2.17.18]
\protect\hypertarget{exr:unnamed-chunk-142}{}\label{exr:unnamed-chunk-142}Determine the closures of the following subsets of the ordered square:
\begin{align}
