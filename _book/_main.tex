...
\protect\hypertarget{exr:unnamed-chunk-104}{}\label{exr:unnamed-chunk-104}Let \(\mathcal{C}\) be a collection of subsets of the set \(X\). Suppose that \$\$\emptyset and \(X\) are in \(\mathcal{C}\), and that finite unions and arbitrary intersections of elements of \(\mathcal{C}\) are in \(\mathcal{C}\). Show that the collection
...
\protect\hypertarget{exr:unnamed-chunk-106}{}\label{exr:unnamed-chunk-106}Show that if \(A\) is closed in \(Y\) and \(Y\) is closed in \(X\), then \(A\) is closed in \(X\).
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-108}{}\label{exr:unnamed-chunk-108}Show that if \(A\) is closed in \(X\) and \(B\) is closed in \(Y\) , then \(A\times B\) is closed in \(X\times Y\)
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-110}{}\label{exr:unnamed-chunk-110}Show that if \(U\) is open in \(X\) and \(A\) is closed in \(X\), then \(U \setminus A\) is open in \(X\), and \(A \setminus U\) is closed in \(X\).
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-112}{}\label{exr:unnamed-chunk-112}Let \(X\) be an ordered set in the order topology. Show that \(\overline{(a, b)} \subseteq [a, b]\). Under what conditions does equality hold?
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-116}{}\label{exr:unnamed-chunk-116}Criticize the following ``proof'' that \(\overline{\cup A_\alpha} \subset \cup \overline{A_\alpha}\):\\
...
\protect\hypertarget{exr:unnamed-chunk-117}{}\label{exr:unnamed-chunk-117}Let \(A\), \(B\), and \(A_{\alpha}\) denote subsets of a space \(X\). Determine whether the following equations hold; if an equality fails, determine whether one of the inclusions \(\supset\) or \(\subset\) holds.
...
\protect\hypertarget{exr:unnamed-chunk-122}{}\label{exr:unnamed-chunk-122}Let \(A \subset X\) and \(B \subset Y\). Show that in the space \(X \times Y\),
...
\protect\hypertarget{exr:unnamed-chunk-124}{}\label{exr:unnamed-chunk-124}Show that every order topology is Hausdorff.
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-10.png}

\begin{exercise}[Mun 2.17.11]
\protect\hypertarget{exr:unnamed-chunk-126}{}\label{exr:unnamed-chunk-126}Show that the product of two Hausdorff spaces is Hausdorff.
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-11.png}

\begin{exercise}[Mun 2.17.12]
\protect\hypertarget{exr:unnamed-chunk-128}{}\label{exr:unnamed-chunk-128}Show that a subspace of a Hausdorff space is Hausdorff.
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-12.png}

\begin{exercise}[Mun 2.17.13]
\protect\hypertarget{exr:unnamed-chunk-130}{}\label{exr:unnamed-chunk-130}Show that \(X\) is Hausdorff if and only if the diagonal \(\Delta = \{x\times x | x \in X\}\) is closed in \(X \times X\).
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-13.png}

\begin{exercise}[Mun 2.17.14]
\protect\hypertarget{exr:unnamed-chunk-132}{}\label{exr:unnamed-chunk-132}In the finite complement topology on \(\mathbb{R}\), to what point or points does the sequence \(x_n = \frac{1}{n}\) converge?
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/ex-14.png}

\begin{exercise}[Mun 2.17.15]
\protect\hypertarget{exr:unnamed-chunk-134}{}\label{exr:unnamed-chunk-134}Show the \(T_1\) axiom is equivalent to the condition that for each pair of points of \(X\), each has a neighborhood not containing the other.
\end{exercise}
...
\protect\hypertarget{exr:unnamed-chunk-139}{}\label{exr:unnamed-chunk-139}Consider the lower limit topology on \(\mathbb{R}\) and the topology given by the basis \(C\) of Exercise 8 of §13. Determine the closures of the intervals \(A = (0, \sqrt{2})\) and \(B = (\sqrt{2}, 3)\) in these two topologies.
\end{exercise}

\includegraphics{figures/Exercises/Ex 2.17/Notsure.png}

\begin{exercise}[Mun 2.17.18]
\protect\hypertarget{exr:unnamed-chunk-141}{}\label{exr:unnamed-chunk-141}Determine the closures of the following subsets of the ordered square:
\begin{align}
